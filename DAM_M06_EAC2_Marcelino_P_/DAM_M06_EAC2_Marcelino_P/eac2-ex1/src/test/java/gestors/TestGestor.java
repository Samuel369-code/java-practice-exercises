...
public class TestGestor {
    ...
    @BeforeAll
    public static void setUpClass() throws SQLException, ManagerException {
        String url = "jdbc:postgresql://localhost:5432/eac2_2526s1";

        String usuari = "ioc";
        String contrasenya = "ioc";
        conn = DriverManager.getConnection(url, usuari, contrasenya);

        gestor = new SongManager(conn);
        createSongs();
    }
    ...
    @Test
    @Order(1)
    public void testDeleteAll() throws ManagerException {
        gestor.deleteAll();
    }
    ...
    public void testAddSong() throws ManagerException {
        for (Song song : songs) {
            gestor.addSong(song);
            assertThrows(ManagerException.class, () -> gestor.addSong(song));
        }
    ...
    @ParameterizedTest
    @ValueSource(strings = {"pop", "country", "heavy metal", "reggaeton"})
    @Order(4)
    public void testGetSongsByTag(String tag) throws ManagerException {
        List<Song> expResult = songs.stream().filter(h -> h.getTags().contains(tag)).toList();

        List<Song> result = gestor.getSongsByTag(tag);
