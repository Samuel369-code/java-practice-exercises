...
import eac5.gestors.PodcastManager;
import eac5.gestors.ManagerException;
import eac5.gestors.ShowcaseManager;
import eac5.gestors.TrackManager;
import eac5.gestors.SongManager;
import eac5.gestors.AudiobookManager;
...
public class TestGestors {
    ...
    private void inicialitzaDades() throws ManagerException {
        netejaBD();
    ...
    private void netejaBD() throws ManagerException {
        trackManager.deleteAll();
        showcaseManager.deleteAll();
    }

    private void createShowcases() {
    ...
    private static void addTracksShowcase() {
        ...
        for (Showcase pl : showcases) {
            ...
            while (selected.size() < 6) {
                ...
                selected.add(t);
                if (!t.getShowcases().contains(pl)) {
                    t.getShowcases().add(pl);
                }
            }
    ...
    public void testGetShowcasesByPublished(boolean published) {
        ...
        List<Showcase> expResult = showcases.stream().filter(s -> s.isPublished() == published).toList();
        assertEquals(expResult.size(), result.size());
        assertEquals(expResult.containsAll(result), true);
    }
    ...
    void testDeleteTrack(int index) throws ManagerException {
        ...
        Track producte = tracks.get(index);
        tracks.remove(producte);
        for (Showcase c : producte.getShowcases()) {
            c.getTracks().remove(producte);
        }

        trackManager.delete(producte.getId());
    ...
    void testGetTracksByExplicit(boolean explicit) throws ManagerException {
        ...
        List<Track> expResult = tracks.stream().filter(t -> t.isExplicit() == explicit).toList();
    ...
    @Order(14)
    void testAddTrackShowcase(int indexCarta, int indexProducte) throws ManagerException {

        Showcase[] showcase = new Showcase[1];
        try {
            showcase[0] = showcases.get(indexCarta);
        } catch (IndexOutOfBoundsException e) {
            showcase[0] = new Showcase();
            showcase[0].setId(-1);
        }
        ...
        Track[] track = new Track[1];
        try {
            track[0] = tracks.get(indexProducte);
        } catch (IndexOutOfBoundsException e) {
            track[0] = new Song();
            track[0].setId(-1);
        }
    ...
    @Order(16)
    void testSetPopularity(int minLikes, int MaxLikes, double popularity) throws ManagerException {

        trackManager.setPopularity(minLikes, MaxLikes, popularity);
